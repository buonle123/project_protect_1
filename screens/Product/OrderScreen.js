...
export default function OrderScreen({ route, navigation }) {
  ...
  const renderItems = () => {
    return CartItem.cart.map((item, i) => {
      return (
        <View
          className="flex-row items-center w-full h-32 px-2 py-4 mt-5 rounded-xl"
        ...
        >
          ...
          <View className="justify-around h-24">
            ...
            <Text>
              price:{" "}
              {(item.product.priceNew * item.quantity).toLocaleString("en-US")}₫
            </Text>
          </View>
        </View>
      );
  ...
  return (
    <View className="flex-1 pt-5">
      <View
        className="flex-row items-center w-full bg-slate-400"
        style={[st.shadowHeader, { height: height * 0.08 }]}
      >
        <TouchableOpacity
          className="items-center justify-center w-10 h-10 ml-4 rounded-xl bg-slate-300 "
          onPress={() => {
            navigation.goBack();
          }}
        >
        ...
        <View style={{ width: width - 112 }} className="">
      ...
      <View
        className="items-center h-full py-5"
      ...
      >
        <View style={{ width: width }} className="items-center">
          <ScrollView className="h-full " style={{ width: width * 0.9 }}>
            ...
            <TouchableOpacity
              className="mt-10 bg-orange-400 h-14 rounded-xl"
              onPress={() => {
                placeAnOder();
              }}
            >
              <Text className="my-auto text-xl font-medium text-center text-white">
                Tổng đơn: {CartItem.totalAmount.toLocaleString("en-US")}₫
              </Text>
            </TouchableOpacity>
          </ScrollView>
        </View>
